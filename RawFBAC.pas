// 2012/13/14/15/16/17 Luciano Olocco
...
uses Lucho, RawFB, RawFBUtils, RawFBServices, URawFBACBrowser, StringListHelper, ComponentBallonHintHelper,
...
const
    CriteriasConst: array [0..6] of string = ('<>', '!', '<=', '<', '>=', '>', '=');

type
   TAddedColumnEvent = function(Sender: TObject; ATable: TObject): String of object;
...
type TRawFBAC = class(TObject)
  private
    ...
    fCalculatedField: array of TCalulatedField;
    fVirtualFieldsEvents: array of TVirtualFieldsEvents;
  ...
  public
    ...
    procedure AddColumn( ColumnCaption: String; Alignment: TAlignment; EventName: TAddedColumnEvent );
    ...
    procedure AddFieldInAM( FieldName, FieldParams, AfterField: String; BeforeEdit, AfterEdit: TNotifyEvent );
end;
...
uses URawFBACExplorer, UGenerators;
...
begin
  ...
  try
    ...
    if GetTableGenerator <> '' then begin
      ...
      fFBClient2.SQL.Add( 'SET GENERATOR ' + GetTableGenerator + ' TO 0;' + #1 );
    end;
...
begin
  ...
  if CtrlName.Length > 3 then begin
    ...
    //ShowMessage( CtrlName );

    for I:= Low(FieldsArray) to High(FieldsArray) do begin
      EnablerStr:= FieldHasEnabler( FieldsArray[I] );
      if EnablerStr.StartsWith( CtrlName + '=', True ) then begin
        EnablerStr:= Copy( EnablerStr, Pos('=', EnablerStr) + 1, EnablerStr.Length );
        if FieldUseComponent('TEdit', CtrlName) then begin
          CtrlEnableBool:= (Sender as TEdit).Text = EnablerStr;
        end
        else begin
...
procedure TRawFBAC.AddColumn( ColumnCaption: String; Alignment: TAlignment; EventName: TAddedColumnEvent );
var NewCant: Integer;
begin
  NewCant:= High(fColumnAddedCaptions) + 1;
...
procedure TRawFBAC.AddFieldInAM(FieldName, FieldParams, AfterField: String; BeforeEdit, AfterEdit: TNotifyEvent);
...
begin
  ...
  for I:= 0 to SL.Count - 1 do begin
    if SL[ I ].StartsWith( UpperCase(AfterField) + '=', True ) then begin
      ...
      if (J = -1) or (J = High(fVirtualFieldsEvents) + 1) then begin
...
begin
  ...
  if CreateNewRecordSet then begin
  ...
  for R:= 0 to RecordCount - 1 do begin
    ...
    for I:= Low(fColumnAddedCaptions) to High(fColumnAddedCaptions) do begin
      F.GridBrowser.Cell[ J + 1 + I, R ].AsString:= fColumnAddedEvents[ I ]( fParent, Self );
    end;
...
begin
  ...
  if Not(Eof) then begin
    if Application.MessageBox('¿Está seguro que desea eliminar el registro?', 'Eliminar registro', MB_YESNO + MB_ICONQUESTION + MB_DEFBUTTON2) = IDYES then begin
      ...
      F.ShowCantidad;
      //F.GridBrowser.RowVisible[ F.GridBrowser.SelectedRow ]:= False;
    end;
  end;
...
begin
  ...
  if NoDuplicateStr <> '' then begin
    fFBClient2.SQL.Text:= 'select * from ' + fTable + ' where ' + DeleteLastString( NoDuplicateStr, ' or ' );
    ...
    if ((fFBClient2.RecordSetCount > 0) and (Afrm.Tag = 0)) or ((fFBClient2.RecordSetCount > 1) and (Afrm.Tag = 1)) then begin
      Application.MessageBox( PChar('No se pueden ingresar valores duplicados'), 'Atención', MB_OK+MB_ICONERROR );
      Exit;
    end;
  end;
...
begin
  ...
  if Not(fFBClient.Eof) then begin
    ...
    if Accept then begin
      ...
      for I:= Low(fColumnAddedCaptions) to High(fColumnAddedCaptions) do begin
        F.GridBrowser.Cell[ High(Fields4BrowserArray) + 2 + I, F.GridBrowser.SelectedRow ].AsString:= fColumnAddedEvents[ I ]( fParent, Self );
      end;
...
begin
  ...
  if Accept then begin
    ...
    for I:= Low(fColumnAddedCaptions) to High(fColumnAddedCaptions) do begin
      F.GridBrowser.Cell[ High(Fields4BrowserArray) + 2 + I, F.GridBrowser.LastAddedRow ].AsString:= fColumnAddedEvents[ I ]( fParent, Self );
    end;
...
constructor TRawFBAC.Create(AOwner: TComponent; Table, INIFileName: String; FDBHostAndFile: String = '');
begin
  ...
  if FieldUseComponent( 'TEdit', FieldName ) then begin
    ...
    if IsModifying then begin
      ...
      if (UniqueField = FieldName) then begin
        ...
        fFBClient2.SQLExecute;
        Edt.Text:= IntToStr(  StrToInt(fFBClient2.Field[ 'GEN_ID' ]) + 1 );
...
begin
  ...
  try
    ...
    fFBClient2.User:= 'SYSDBA';
    fFBClient2.PassWord:= 'masterkey';
...
begin
  Result:= fINI.ReadString( fTableInINI, 'Fields4Browser', '' );
end;
...
begin
  fFBClient2.SQL.Text:= 'SELECT GEN_ID( ' + GetTableGenerator + ', 0) FROM RDB$DATABASE';
  fFBClient2.SQLExecute;
  Result:= StrToInt(fFBClient2.Field[ 'GEN_ID' ]) + 1;
  fFBClient2.RollBackTransaction;
end;
  ...
  if (I > -1) then begin
    if FieldType[ FieldName ] = 'BL' then begin
      ...
      if (FieldType[ FieldName ] = 'TS') and (fDataValues[ i ] <> '') and (LowerCase(fDataValues[ i ]) <> 'now') then begin
        Result:= FormatDateTime( 'dd-mm-yyyy, HH:mm:ss.zzz', StrToDateTimeMillisecond( fDataValues[ i ] ) );
      end
      else begin
...
begin
  Result:= fINI.ReadString( fTableInINI, 'Title', '' );
end;
...
begin
  try
    fFBClient2.SQL.Text:= 'SELECT GEN_ID( ' + GeneratorName + ', 1) AS GEN FROM RDB$DATABASE';
    fFBClient2.SQLExecute;
    Result:= StrToInt(fFBClient2.Field[ 'GEN' ]);
  except
...
function TRawFBAC.ReadStringAndResolve(const Section, Ident, Default: String): String;
var AData: String;
...
begin
  ...
  ShowMessage( 'Importando datos: Errores: ' + IntToStr( Errores ) );
...
begin
  ...
  fDefault4Browse:= fINI.ReadString( fTableInINI, 'Default4Browse' , '' );
...
begin
  ...
  if (Not(FShowDeleted)) and (Not(StringStartWith( TheSQL, 'UPDATE OR INSERT' ))) then begin
    ...
    if ((DeletedPos = 0) or (WherePos > DeletedPos)) then begin
      if (Copy(Trim(LowerCase(TheSQL)), 1, 6 ) = 'select') or (Copy(Trim(LowerCase(TheSQL)), 1, 6 ) = 'update') then begin
        HasColon:= Pos( ';', TheSQL ) > 0;

        TheSQL:= StringReplace( TheSQL, ';', ' ', [rfReplaceAll] );
        ...
        {end
          ...
          QueLetra:= Copy( TheSQL, Pos( ' FROM ', UpperCase(TheSQL) ) + 6, Length( TheSQL ) );
          QueLetra:= Copy( QueLetra, Pos( ' ', TheSQL ), Length( TheSQL ) );
          if Not(StrStartWithAnyOfArray( QueLetra, I, AddStrToVector('where ', Orden) )) then begin
            QueLetra:= QueLetra;
          end; }
        end;
        if QueLetra <> '' then begin
          Deleted:= QueLetra + Deleted;
        end;

        W:= Pos( 'where ', LowerCase( TheSQL ) );
  ...
  for I in fBrowserCtrlsRedirectorList do begin
    if (I.AEdit = Sender as TEdit) then begin
      if (I.AEdit.Text <> '')and ((I.AFirebirdTable as TRawFBAC).CreateRecordSetAndCount( I.BrowseField + '=' + I.AEdit.Text, '*' ) = 1) then begin
        I.APanel.Caption:= ' ' + (I.AFirebirdTable as TRawFBAC).Field[ I.ShowField ];
      end
      else begin
        I.APanel.Caption:= '';
      end;
